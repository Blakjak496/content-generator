...
###3. Create a config.json file
{
  ...
  "lastRun": null,
  "newFileMarker": "STORY",
  "sectionHeadings": ["Headline", "Subdeck", "Article", "Newsletter Summary", "LinkedIn Post"]
}
...
###6. Schedule
The script is stateless between runs aside from config.json. It can be scheduled using cron or any equivalent task scheduler. The articleCounter increments by the number of documents produced per run and is only written back on a successful run, so a failed run does not advance the counter.
State management
